document.write('<script src="/script-base.js?v=20260817-seven"><\/script>');

(function(){
  if(!window.location.pathname.includes('/green/')) return;

  // Verified Green collection assets. Keep the Infinity Bracelet pinned to this exact file.
  const greenPieces = [
    {product:1, src:'E457D36E-5F1A-4B9F-81D8-C728FC636D9C.png?v=green9'},
    {product:0, src:'7962FDDD-FF5A-4621-85D1-BAA0AE1BD508.png?v=green9'},
    {product:4, src:'61AC2F61-DDC9-4E44-AB54-A99A268A681D.png?v=green9'},
    {product:2, src:'4907FEF0-19B9-4690-8B51-5B2FA320AD58.png?v=green9'},
    {product:3, src:'19CE06DF-C75C-4676-9477-ADB000875EB6.png?v=bracelet-final-20260817'}
  ];
  ...
  function setGridColumns(grid,count){
    ...
    grid.style.gridTemplateColumns=w<=760?'1fr':`repeat(${count},minmax(0,1fr))`;
    grid.style.gap=w<=760?'18px':'14px';
  }

  function makeConceptCard(piece,template){
  ...
  function applyGreenLayout(){
    ...
    // Keep the five Green collection cards, including the verified bracelet.
    const ordered=greenPieces.map(piece=>byProduct.get(piece.product)).filter(Boolean);
  ...
  function applyConceptLayout(){
    ...
    if(!dnaPendant) dnaPendant=makeConceptCard(conceptPieces[1],template);
    if(!dnaBracelet) dnaBracelet=makeConceptCard(conceptPieces[3],template);
    byProduct.set(8,dnaPendant);byProduct.set(9,dnaBracelet);
    Array.from(grid.querySelectorAll('.product-card')).forEach(c=>c.remove());
    conceptPieces.forEach(piece=>{
      const card=byProduct.get(piece.product);if(!card)return;
  ...
  // Green page currently displays the verified five-piece Green collection.
  function scheduleLayout(){[80,240,700].forEach(ms=>setTimeout(applyGreenLayout,ms));}
  ...
  function installHeroCTAButtons(){
    ...
    [{label:'Discover Collection',target:'productGrid',left:'7.8%',top:'64.0%',width:'20.0%',height:'9.5%'},{label:'Shop Now',target:'shop',left:'29.0%',top:'64.0%',width:'15.5%',height:'9.5%'}].forEach(spec=>{
      ...
      link.addEventListener('click',e=>{e.preventDefault();const target=document.getElementById(spec.target);if(!target)return;target.scrollIntoView({behavior:'smooth',block:'start'});history.replaceState(null,'',window.location.pathname+window.location.search+(spec.target==='productGrid'?'#productGrid':'#shop'));});frame.appendChild(link);
    });
  }
  ...
  window.addEventListener('resize',()=>{const grid=document.getElementById('productGrid');if(grid)setGridColumns(grid,5);},{passive:true});
  ...
  document.addEventListener('click',e=>{
    ...
    const card=button.closest('.product-card');const product=Number(card&&card.dataset.product);
    const piece=greenPieces.find(x=>x.product===product)||conceptPieces.find(x=>x.product===product);if(!piece)return;
    setTimeout(()=>{const modalImg=document.getElementById('conceptImage');if(modalImg)modalImg.src=piece.src;},0);
  });
})();
