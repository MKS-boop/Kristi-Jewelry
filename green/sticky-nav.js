(() => {
  ...
  const openCart=()=>{if(!cart)return;searchBox?.classList.remove('is-open');menuBox?.classList.remove('is-open');cart.classList.add('is-open');cart.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';};
  ...
  $('[data-add-crown]')?.addEventListener('click',()=>{if(cartContent){cartContent.innerHTML='<div class="cart-item"><strong>Emerald Crown</strong><p>Kristi &amp; G · Armenia</p><p>$2,500.00</p></div><a class="button secondary" href="#shop" data-cart-shop>Continue Shopping</a>';cartContent.querySelector('[data-cart-shop]')?.addEventListener('click',e=>{e.preventDefault();closeCart();scrollToTarget('#shop');});}openCart();});
